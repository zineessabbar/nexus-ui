...
export const useStore = create<Store>()(
  persist(
    (set) => ({
      ...
      selectedEndpoint: 'http://127.0.0.1:8000/api/v1',
      ...
      // Replaces nuqs query params — simple state
      agentId: null,
      ...
      updateLastAgentMessage: (content: string) => set((state) => {
        const messages = [...state.messages]
        const lastAgentIdx = messages.map(m => m.role).lastIndexOf('agent')
        if (lastAgentIdx !== -1) {
          messages[lastAgentIdx] = { ...messages[lastAgentIdx], content }
        }
        return { messages }
      })
    }),
